...
connection = mysql.createConnection({
  ...
  user: 'root',
  password: '12345',
  database: 'bdgasolinera'
});

let userModel = {};

userModel.get = (callback) => {
  if (connection) {
    connection.query('SELECT * FROM gasolinera',
      (err, rows) => {
        if (err) {
          throw err
        }
...
userModel.insertarGasolinera = (  userData, callback) => {
  try {
    if (connection) {
      connection.query('INSERT INTO Gasolinera SET ?', userData,
        (err, result) => {
          if (err) {
            throw err;
          } else {
